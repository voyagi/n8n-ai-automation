...
async function main() {
  ...
  console.log('='.repeat(80));
  console.log(`Mode: ${RAPID_FIRE ? 'RAPID-FIRE (concurrent)' : 'SEQUENTIAL (1s delays)'}`);
  ...
  if (RAPID_FIRE) {
    ...
    for (let i = 0; i < testData.length; i++) {
      ...
      console.log(`[${i + 1}/${testData.length}] Submitting: ${data.test_case}...`);
      ...
      if (result.success) {
        ...
        console.log(`  ${categoryIcon} Success (${formatDuration(result.duration)}) - Category: ${result.result.category} (expected: ${data.expected_category})`);
      } else {
        console.log(`  ✗ Failed (${formatDuration(result.duration)}) - ${result.error}`);
      }
  ...
  const categoryMatchCount = results.filter(r => r.success && r.categoryMatch).length;
  const avgDuration = results.reduce((sum, r) => sum + r.duration, 0) / results.length;

  console.log(`Total submissions: ${results.length}`);
  console.log(`✓ Successful: ${successCount}`);
  console.log(`✗ Failed: ${failCount}`);
  console.log(`Category accuracy: ${categoryMatchCount}/${successCount} (${((categoryMatchCount / successCount) * 100).toFixed(1)}%)`);
  ...
  if (mismatches.length > 0) {
    console.log();
    console.log('CATEGORY MISMATCHES (informational only - AI variation is acceptable):');
    mismatches.forEach(r => {
      console.log(`  • ${r.data.test_case}: expected "${r.data.expected_category}", got "${r.result.category}"`);
    });
    console.log();
  }
